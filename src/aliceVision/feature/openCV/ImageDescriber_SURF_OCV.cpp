...
void SURF_openCV_Params::setConfigurationPreset(ConfigurationPreset preset)
{
                                          ...
                                          const image::Image<unsigned char> * mask)
{
  ...
  detector->detect(img, v_keypoints);
  ...
  detector->compute(img, v_keypoints, m_desc);
  ...
  cv::Mat m_sum;
  cv::reduce(m_desc, m_sum, 1, cv::REDUCE_SUM);
  ...
  for(const auto& i_kp : v_keypoints)
  {
    ...
    {
      desc[j] = static_cast<unsigned char>(512.0*sqrt(m_desc.at<float>(cpt, j)/m_sum.at<float>(cpt, 0)));
    }
