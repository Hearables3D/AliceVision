...
public:
  ...
  double hessianThreshold = 100;    //< default opencv value is 100
  int nOctaves = 4;                 //< default opencv value is 4
  int nOctaveLayers = 3;            //< default opencv value is 3
  bool extended = true;             //< default opencv value is false
  bool upright = false;             //< default opencv value is false
  //bool rootSift = true;
};

/**
 * @brief Create an ImageDescriber interface for OpenCV SURF feature extractor
 * Regions is the same as classic SURF : 128 unsigned char
 */
