 ...
 */
class ImageDescriber_Float256 : public ImageDescriber
{
public:
  explicit ImageDescriber_Float256(const SiftParams& params = SiftParams(), bool isOriented = true)
    : ImageDescriber()
    , _params(params)
  ...
  ~ImageDescriber_Float256() override
  {
  ...
  {
    return EImageDescriberType::FLOAT_256;
  }
  ...
  std::size_t getMemoryConsumption(std::size_t width, std::size_t height) const override
  {
    ...
    throw std::runtime_error("Float 256 extraction is not implemented.");
  }
  ...
  {
    regions.reset(new Float_256_Regions);
  }
